...
def generate_launch_description():
    ...
    #wamv_models_path = os.path.join(pkg_share, 'models')
    #wamv_model_name = 'wamv_gazebo.urdf.xacro'
    #wamv_model_path = os.path.join(wamv_pkg_share,'urdf',wamv_model_name)
    wamv_model_name = 'QR_wamv'
    wamv_model_path = os.path.join(pkg_share,'models',wamv_model_name,'model.sdf')
    print(wamv_model_path)
    
    #doc = xacro.process_file(wamv_model_path, mappings={'arucos_enabled':'True'})
    ...
    add_wamv_gazebo_cmd = Node(package='gazebo_ros', executable='spawn_entity.py',
                      arguments=['-entity', 'WAM-V', '-file', wamv_model,
                                 '-x','-10',
                                 '-y','0',
                                 '-z','0'],
                      output='screen')
